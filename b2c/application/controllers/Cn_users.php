...
class Cn_users extends CI_Controller {
    ...
    public function my_orders()
    {
       ...
       if(empty($customer_id)){
        ...
        /*Get order list*/
        $table = "order_dtl"; 
        $condition = " ( customer_id = '$customer_id' OR flag_strategic_custome_id = '$customer_id'  ) AND crm_status = '7' ";
    ...
    $customer_id = $_SESSION["customerId"];
       if(empty($customer_id)){
        ...
        //
        $data['status_summary_design'] = $this->Md_customer->get_status_summary($order_id,'design');   
        $data['status_summary_execution'] = $this->Md_customer->get_status_summary($order_id,'execution');   
        $data['status_summary_procurement'] = $this->Md_customer->get_status_summary($order_id,'procurement'); 
        //
        $data['detailed_status_design'] = $this->Md_customer->get_detailed_status($order_id,$section_id,'design');   
        $data['detailed_status_execution'] = $this->Md_customer->get_detailed_status($order_id,$section_id,'execution');   
        $data['detailed_status_procurement'] = $this->Md_customer->get_detailed_status($order_id,$section_id,'procurement');
        //
        $data['milestone_details_design'] = $this->Md_customer->get_milestone_details($order_id,'design');   
        $data['milestone_details'] = $this->Md_customer->get_milestone_details($order_id,'execution');   
        $data['milestone_details_procurement'] = $this->Md_customer->get_milestone_details($order_id,'procurement');  
