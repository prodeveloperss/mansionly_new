    ...
    <div class="container">
      <div class="row">
        <div class="col-md-9 col-sm-9 col-xs-12">
		  ...
		  <div class="d-profile-Img"> <img src="<?php echo image_url; ?>/media/images/designer-images/150X150/<?php echo $designer_details[0]['designer_logo'];?>"> </div>
		  <?php }?>
          <div class="d-profile-text clearfix">
            ...
            <a class="conectBtn" href="javascript:void()" data-toggle="modal" data-target="#ConatctMe"> Contact</a> </div>
        </div>
        <?php if(!empty ($designer_details[0]['designer_logo2'])) { ?>
        <div class="col-md-3 col-sm-3 col-xs-12">
          <div class="profile-logo"> <img src="<?php echo image_url; ?>/media/images/designer-images/<?php echo $designer_details[0]['designer_logo2'];?>"  class="img-responsive"> </div>
        </div>
        <?php } ?>
      </div>
    </div>
      ...
      <div class="profile-des clearfix">
        <ul>
          <?php if(!empty($designer_details[0]['designer_description'])) { 
              $char_limit = 100;
          ?>
          <li> <i class="fa fa-user-circle-o" style="display:block !important;"></i> <strong> Profile</strong> <span class="less"><?php echo substr(preg_replace('#<[^>]+>#', ' ', $designer_details[0]['designer_description']),0,$char_limit); ?>
            <a style="cursor:pointer; <?php if(strlen(strip_tags($designer_details[0]['designer_description']))<= $char_limit){?> display:none; <?php } ?>" class="read_more">...Read More</a></span> <span class="more" style="display:none;"><?php echo $designer_details[0]['designer_description']; ?><br>
            <a style="cursor:pointer;" class="read_less">Read Less</a></span> </li>
          <?php } if(!empty($designer_details[0]['design_philosophy'])) { ?>
          <li> <i class="fa fa fa-diamond"></i> <strong> Design Philosophy</strong> <span class="less"><?php echo substr(preg_replace('#<[^>]+>#', ' ', $designer_details[0]['design_philosophy']),0,$char_limit); ?>
          <a style="cursor:pointer; <?php if(strlen(strip_tags($designer_details[0]['design_philosophy']))<= $char_limit){?> display:none; <?php } ?>" class="read_more"  style="display:none;" > ...Read More</a></span> <span class="more" style="display:none;"><?php echo $designer_details[0]['design_philosophy']; ?><br>
            <a style="cursor:pointer;" class="read_less">Read Less</a></span> </li>
          <?php } if(!empty($designer_details[0]['design_awards'])) { ?>            
          <li> <i class="fa fa-trophy"></i> <strong> Awards</strong> <span class="less"><?php echo substr(preg_replace('#<[^>]+>#', ' ', $designer_details[0]['design_awards']),0,$char_limit); ?>
            <a style="cursor:pointer; <?php if(strlen(strip_tags($designer_details[0]['design_awards']))<= $char_limit){?> display:none; <?php } ?>" class="read_more"> ...Read More</a></span> <span class="more" style="display:none;"><?php echo $designer_details[0]['design_awards']; ?><br>
            <a style="cursor:pointer;" class="read_less">Read Less</a></span> </li>
          <?php } ?>
        </ul>
      </div>
          ...
          <div role="tabpanel" class="tab-pane fade <?php if(($flag=="executionportfolio") || ($flag=="")) { ?>in active<?php } ?>" id="home">
            <div id="append_portfolio">
              ...
              <div class="col-sm-6 col-md-4 col-xs-12  extraPd" >
                <div class="design-profile-box"> <a class="likeico" href="javascript:void(0);"><i id="<?php echo 'executionportfolio'.$row['id']; ?>" class="fa fa-heart <?php if(in_array($row['id'], $customerFavoriteExecutions)){echo ' heartRed';}?>" aria-hidden="true" onclick="saveFavorite('Portfolio','executionportfolio', <?php echo $row['id'];?> );"></i></a>
                  ...
                  <div class="owl-carousel owl-demo">
                    ...
                    <div class="item">
                       <?php
                        ...
                        <a class="add_exe_url" href="<?php echo base_url().strtolower(urlencode(str_replace($url_replace_char_array, '-',trim($row['designer_name'])))).'/'.strtolower(urlencode(str_replace($url_replace_char_array, '-',trim($row['portfolio_name'])))).'/ep/'.$row['id'].'?q='.$q;?>">
                            <img src="<?php echo image_url;?>media/images/master-execution-images/<?php echo $image_size.$row['master_image']; ?>" >
                        </a> 
                    </div>
                    <?php
                        if(!empty($row['secondary_images'])){ 
                    ...
                    <div class="item"> 
                        <a class="add_exe_url" href="<?php echo base_url().strtolower(urlencode(str_replace($url_replace_char_array, '-',trim($row['designer_name'])))).'/'.strtolower(urlencode(str_replace($url_replace_char_array, '-',trim($row['portfolio_name'])))).'/ep/'.$row['id'].'?q='.$q;?>">
                            <img src="<?php echo image_url;?>media/images/master-execution-images/<?php echo $image_size.$row_image; ?>" >
                        </a>
                    </div>
                    <?php  } } ?>
                    <?php   if(!empty($row['secondary_images'])){ ?>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-body">
        ...
        <div class="col-sm-5">
          <div class="design-modal-img clearfix"> <img src="<?php echo image_url; ?>/media/images/designer-images/150X150/<?php echo $designer_details[0]['designer_logo'];?>" > </div>
          <div class="clearfix"></div>
...
function getDefaultDesignerProfileGallery()
{
   ...
   $('#loadMore').hide();
    ...
    $.ajax({
        ...
        type:"post",
        data:{'offset':offset,'designer_id':designer_id,'customerFavoriteThemes':customerFavoriteThemes,'q':q},
        success: function(response){
 var data = response.split('|*|*|');        
