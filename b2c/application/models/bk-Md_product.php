...
class Md_product extends CI_Model {
    ...
    public function productTotalCountByCategoryIds($arrSubCatIds)
	{
 ...
 public function productListByCategory($arrSubCatIds,$offset,$limit)
	{
   ...
   public function productListBySection($section_id,$offset,$limit)
	{
...
//                $limit = 24;
		...
		$queryStr = " select t1.*, brand_name, brand_image, brand_id from  jb080_ecom_products t1
		...
		return $query->result_array();
          ...
          public function productTotalCountBySectionId($section_id)
	{
            $this->db->select('COUNT(`t1`.`product_id`) AS `totalCount`');
            $this->db->from('ecom_products AS `t1`');
